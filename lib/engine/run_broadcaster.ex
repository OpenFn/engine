defmodule Engine.RunBroadcaster do
  @moduledoc """
  ...
  alias Engine.{Config, Matcher, RunDispatcher, Run, JobStateRepo}
  ...
  def handle_call({:process_run, %Run{job: job} = run}, _from, state) do
    ...
    runs =
      ...
      |> Enum.filter(fn {_job, trigger} ->
        (run.result.exit_code == 0 && trigger.success) ||
          (run.result.exit_code > 0 && trigger.failure)
      end)
